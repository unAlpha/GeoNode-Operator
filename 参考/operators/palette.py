...
class NODEBOOSTER_OT_initalize_palette(bpy.types.Operator, ):
    ...
    def initialize_palette(self, palette_name:str="MyPalette", reuse_existing:bool=True):
        ...
        if (pal is None) or (not reuse_existing):
    ...
    def execute(self, context,):
        ...
        #fill a property, passed via ui layout.context_pointer_set
        context = bpy.context	
        if hasattr(context, "scene_context"):
            context.scene_context.palette_assign_ptr = self.initialize_palette(palette_name='AssignPalette', reuse_existing=False,)
        if hasattr(context, "node_context"):
            context.node_context.palette_ptr = self.initialize_palette(palette_name='NodePalette', reuse_existing=False,)
